...
def _fetch_cookies_via_playwright_impl() -> list[dict]:
    """Ouvre Chrome pour passer le challenge F5 et renouveler les cookies."""
    seed_url = (
...
def _fetch_cookies_via_playwright() -> list[dict]:
    if _running_in_async_loop():
        with ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(_fetch_cookies_via_playwright_impl).result()
    return _fetch_cookies_via_playwright_impl()
